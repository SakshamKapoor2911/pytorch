...
class TestDecomposeShardDimAllToAll(TestCase):
    ...
    def _run_decomp_locally(
        ...
        *,
        ...
        outputs = []
        resolve_group, get_group_size = self._group_mocks(group_size)
        with config.patch({"decompose_shard_dim_alltoall": True}):
            with resolve_group, get_group_size:
                with (
                    mock.patch.object(
    ...
    def _group_mocks(self, group_size: int):
        return (
            mock.patch(
    ...
    def _trace_decomp(
        ...
        *,
        ...
        def fn(x):
            return decomp(x, gather_dim, shard_dim, "test_pg")

        resolve_group, get_group_size = self._group_mocks(group_size)
        with config.patch({"decompose_shard_dim_alltoall": True}):
            with resolve_group, get_group_size:
                return make_fx(fn, tracing_mode="fake")(inp)
